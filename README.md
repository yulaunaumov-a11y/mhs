...
## Данные
Данные представлены в виде снимков БПЛА с разрешением 0,6 м/пиксель. Один снимок размера 8192x8192 пикселей. Снимки представляют из себя городскую или сельскую зайстроку. Поскольку каждый снимок крупного размера, в среднем 8192x8192, то каждый снимок необходимо разделить на патчи. В ходе эксперементов было установлено, что наиболее качественный результат получается с патчем 1024х1024.
Примеры результатов:
![Example1](images/Example1.png)

## Метрики-результаты
В ходе обучения модели были досутигнуты результаты. Когда каждый iou класса > 0.9, метрика F1 >= 0.95 для каждого класса. 
Пример метрики:
![Example2](images/All_metric.png)
